...
export class InvoiceLineItemComponent implements OnInit, OnDestroy {
  ...
  transition() {
    this.router.navigate(['/invoice-line-item'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  registerChangeInInvoiceLineItems() {
    this.eventSubscriber = this.eventManager.subscribe('invoiceLineItemListModification', response => this.loadAll());
  }
  ...
  protected paginateInvoiceLineItems(data: IInvoiceLineItem[], headers: HttpHeaders) {
